...
void main(){
  mat4 skin = getBoneMatrix(a_JOINTS_0[0]) * a_WEIGHTS_0[0] +
...
export async function startSkinEngine(
  ...
  const skinPI = wu.createProgramInfoFromSources(gl, [skinVS, fs]);
  const meshPI = wu.createProgramInfoFromSources(gl, [meshVS, fs]);
  ...
  class Skin {
    ...
    update(node){
      const invScene = m4.inverse(node.worldMatrix);
      for (let j=0;j<this.joints.length;++j){
        const dst = this.jointMatrices[j];
        m4.multiply(invScene, this.joints[j].worldMatrix, dst);
        m4.multiply(dst, this.invBind[j], dst);
      }
      gl.bindTexture(gl.TEXTURE_2D, this.texture);
      gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,4,this.joints.length,0,
                    gl.RGBA,gl.FLOAT,this.texData);
    }
  }
  ...
  gltf.scenes.forEach(sc=>{
    sc.root=new Node(new TRS(),sc.name);
